    ...
    return new Promise((resolve) => {
      ...
      if(hasType){
        ...
        for(let index in value){
          ...
          if(fieldChild._multiple === true){
            let itemValue = item;
            let cleanFieldChild = Object.assign({}, fieldChild);
            delete cleanFieldChild['_multiple'];
            let hasDeepLevel = Object.keys(cleanFieldChild).length > 0;

            if(itemValue.hasOwnProperty("link_type") && itemValue.link_type === "Document"){
              promises.push(this._getSubDocument(itemValue.id, cleanFieldChild, options));

            }else if(typeof itemValue === "object" && hasDeepLevel){
              promises.push(this._extract(itemValue, cleanFieldChild, options));
  ...
  _multipleGroup(item, fieldChild, options){
    return new Promise( (resolve) => {
      ...
      Object.keys(item).forEach( itemChildKey => {
        ...
        let hasDeepLevel = Object.keys(itemChildKey).length > 0;
        ...
        }else if(typeof itemFieldChild === "object" && hasDeepLevel){
          childPromises.push(new Promise( (resolve) => {
